...
const ENTRIES = [
  ...
  },
  {
    ...
    date: "Apr 9, 2026",
    tag: "Protocol",
    title: "Solana devnet + batched settlement",
    items: [
      "Solana devnet is now a first-class settlement target — configure with `{ chain: 'solana-devnet' }` for local and staging environments.",
      "Batched settlement mode added: amortize a single Solana transaction across many calls, reducing per-call fee cost for high-volume resources.",
      "Quote responses now include a `feeEstimate` field with the expected Solana transaction fee in USDC terms.",
      "Improved verifier latency — nonce registry lookups are now cached, reducing round-trip time by ~30ms on repeat callers.",
    ],
  },
  {
    version: "v0.402.1",
  ...
  },
  {
    ...
    date: "Mar 14, 2026",
    tag: "Protocol",
    title: "Public beta — Solana settlement live",
    items: [
      "Astro protocol enters public beta. Breaking changes will be communicated with ≥ 14 days notice from this release forward.",
      "Solana settlement programs deployed to mainnet. SOL and USDC supported as settlement assets.",
      "TypeScript SDK `@astro/sdk` published to npm.",
      ...
      "Receipt indexer live — all settled calls are indexed and queryable via the Console API within 2 seconds of Solana finality.",
    ],
  },
  {
    version: "v0.3.1",
