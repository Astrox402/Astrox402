...
export const Route = createFileRoute("/docs/faq")({
  head: () => ({
    meta: [
      { title: "FAQ — Meridian Docs" },
      { name: "description", content: "Frequently asked questions about the Meridian payment-native protocol — pricing, chains, agents, security, and operations." },
      { property: "og:title", content: "FAQ — Meridian Docs" },
      { property: "og:description", content: "Common questions about the Meridian protocol, SDKs, and operations." },
    ],
  }),
  component: FaqPage,
});
  ...
  return (
    <DocsLayout onThisPage={TOC}>
      <PageHeader
        ...
        intro="Short answers to the questions teams ask most often when adopting Meridian. For deeper context, follow the linked sections — every answer here is backed by a more detailed reference page."
      />

      <DocSection id="general" title="General">
        <QA q="Do I need to know anything about crypto to use Meridian?">
          <p>No. If you can write a fetch handler and read a function signature, you can ship a Meridian endpoint. The protocol uses Ethereum for settlement, but the SDK abstracts every blockchain interaction. You declare a price in USDC, the SDK handles the rest.</p>
        </QA>
        <QA q="Is Meridian a hosted service?">
          <p>Partially. The SDK and the settlement contracts are open-source and require no Meridian-hosted infrastructure to operate. The dashboard, indexer, and reconciliation API are hosted conveniences; you can replace any of them with your own implementation against the public protocol.</p>
        </QA>
        <QA q="What does '402-inspired' mean?">
          <p>The HTTP status code 402 ("Payment Required") was reserved in the original HTTP spec but never standardized. Meridian operationalizes it: a request without a valid payment intent receives 402 with a structured quote, and the client signs and retries. See <Mono>/docs/handshake</Mono>.</p>
        </QA>
      </DocSection>

      <DocSection id="pricing-q" title="Pricing & fees">
        <QA q="What does Meridian charge?">
      ...
      <DocSection id="agents-q" title="Agents">
        ...
        <QA q="Can agents call each other?">
          <p>Yes. Agent-to-agent commerce is just one Meridian endpoint calling another. The receipt chain makes the entire delegation graph reconstructable after the fact.</p>
        </QA>
      </DocSection>
      ...
      <DocSection id="ops" title="Operations">
        ...
        <QA q="What happens if my handler throws after payment is verified?">
          <p>The SDK automatically triggers an onchain refund and returns a typed <Mono>MeridianHandlerError</Mono> to the caller. The original payment is reversed in the same atomic settlement; the caller is never charged for a failed handler.</p>
        </QA>
      ...
      <DocSection id="compare" title="Comparisons">
        <QA q="How is this different from Stripe?">
          <p>Stripe operates a private ledger and a closed network of acquirers. Meridian operates an open protocol with public, onchain settlement. Stripe is the right answer for human checkout; Meridian is the right answer for machine-to-machine commerce, programmable APIs, and verifiable settlement.</p>
        </QA>
        <QA q="How is this different from API keys?">
          <p>API keys are a static authorization that, once issued, can be used unlimitedly until revoked. Meridian intents are single-use authorizations bound to an exact request. Keys leak; intents cannot be replayed. Keys require revocation; intents expire on their own.</p>
        </QA>
        <QA q="Why not just use a normal payment processor with metering?">
          <p>You can. But you'll be running an entitlement service, a metering pipeline, a billing engine, a dispute system, and a reconciliation tool — and your customers will still have to trust that your dashboard is accurate. Meridian replaces that stack with one HTTP exchange and a public chain.</p>
        </QA>
      </DocSection>
